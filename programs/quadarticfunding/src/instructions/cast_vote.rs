...
pub fn cast_vote(ctx: Context<CastVote>, vote_type: u8) -> Result<()> {
    ...
    if vote_type == 1 {
        ctx.accounts.proposal.yes_vote_count = ctx.accounts.proposal.yes_vote_count.checked_add(voting_credits).unwrap();
    } else {
        ctx.accounts.proposal.no_vote_count = ctx.accounts.proposal.no_vote_count.checked_add(voting_credits).unwrap();
    }
